...
df_subset <- df_filtered[names(clusters)]

lapply(setNames(seq_along(1:length(clusters)), names(clusters)), function(barcode){
    df_subset[[barcode]] %>% filter(allele %in% clusters[[barcode]])
}) -> df_subset
...
lapply(df_subset, function(df){
  ...
  Reduce(function(...) merge(..., all=T), .) %>% 
  group_by(id) %>% 
  mutate(allele = paste0("allele_", row_number())) %>% 
  pivot_wider(names_from = allele, values_from = mode) -> mode_df
