...
```{r}
file <- "../data/out/barcode14.hg19.strique.tsv"
...
process_file <- function(file_path) {
  ...
  mode_df <- df_filtered %>%
    ...
    mutate(allele_ref = seq.int(nrow(.)))
...
result_df <- lapply(file_paths, function(file_path){
  ...
  Reduce(function(...) merge(..., all=T), .) %>% select(-c(2)) %>% 
  pivot_wider(names_from = allele_ref, values_from = c(mode_repeat, repeat_freq, allele_freq, prop))
...
write.csv(result_df, paste0("results/", Sys.Date(), "FGF14_result.csv"))
